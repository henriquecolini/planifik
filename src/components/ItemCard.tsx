...
export function ItemCard({
  ...
  const dueLabelText = (() => {
    if (!due) return null;
    switch (due.type) {
      case "today":            return t("dueToday");
      case "tomorrow":         return t("dueTomorrow");
      case "inDays":           return t("dueInDays",       { n: String(due.days) });
      case "on":               return t("dueOn",           { date: due.formattedDate });
      case "expiredYesterday": return t("expiredYesterday");
      case "expiredDaysAgo":   return t("expiredDaysAgo",  { n: String(due.days) });
    }
  })();

  // ── Pay button label ───────────────────────────────────────────────────────
  const actionLabel = isPaid
    ? (isIncome ? t("received") : t("paid"))
    : (isIncome ? t("receive")  : t("pay"));
  ...
  return (
    <div className={cn(
        ...
        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
          ...
          {due && dueLabelText && !isPaid && (
            <span className={cn(
              ...
              due.isExpired  ? "text-bill bg-bill-bg"
              : due.isUrgent ? "text-warning bg-warning-bg"
      ...
      {editing ? (
        ...
        <button
          onClick={() => { if (!isPaid) setEditing(true); }}
          disabled={isPaid}
        ...
        >
          {signChar}{formatCurrency(absForDisplay)}
        </button>
      )}
        ...
        {menuOpen && (
          <>
            ...
            <div className="absolute right-0 top-8 w-40 bg-white border border-border-default rounded-xl shadow-lg overflow-hidden z-40 animate-scale-in">
              ...
              {isPaid && (
                <button
                  onClick={() => { onUnpay(item); setMenuOpen(false); }}
                  className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-text-secondary hover:bg-elevated transition-colors"
                >
