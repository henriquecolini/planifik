    ...
    return (
        <div className={cn(
          ...
          {total >= 0 ? "+" : "−"}{formatCurrency(Math.abs(total))}
            ...
            {/* ── Items ── */}
            {showItems && (
                <div className="px-3 pb-3 space-y-1.5 border-t border-border-subtle pt-2.5">
                    {children}
                </div>
            )}
        </div>
    ...
    return (
        <div className="rounded-2xl border border-accent bg-white shadow-xl ring-1 ring-accent/30 opacity-90">
            <div className="flex items-center gap-2 px-3 py-2.5 select-none">
                ...
                <span className="flex-1 text-sm font-semibold text-text-primary truncate">{folder.name}</span>
                <span className={cn("text-sm font-semibold tabular-nums", total >= 0 ? "text-income" : "text-bill")}>
          {total >= 0 ? "+" : "−"}{formatCurrency(Math.abs(total))}
        </span>
            </div>
        </div>
