...
import { Modal, Button, Toggle } from "@/components/ui/index";
...
interface PayItemModalProps {
  ...
  month: string;
  accounts: Item[];
  onConfirm: (paymentMethod: PaymentMethod | null, paymentItemId: string | null, deductBalance: boolean) => Promise<void>;
}

export function PayItemModal({ open, onClose, item, month, accounts, onConfirm }: PayItemModalProps) {
  const { t } = useI18n();
  ...
  const handleConfirm = async () => {
    ...
    try {
      const method: PaymentMethod | null = selectedAccount
        ? (selectedAccount.type === "CREDIT_CARD" ? "credit_card" : "checking_account")
        : null;
  ...
  const AccountOption = ({ acc }: { acc: Item }) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-xs text-text-muted tabular-nums">{formatCurrency(balanceAmount)}</span>
      </button>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 bg-elevated rounded-xl px-3 py-3 border border-border-subtle">
        {isAccountItem(item) ? <BankIcon bank={item.bank} size="md" /> : <ItemIcon icon={item.icon} type={item.type} size="md" />}
        ...
        checkingAccounts.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-xs font-medium text-text-secondary">
              {t("depositInto")} <span className="text-text-muted font-normal">({t("optional")})</span>
            </p>
            {checkingAccounts.map((acc) => <AccountOption key={acc.id} acc={acc} />)}
          </div>
        )
      ) : (
        <div className="space-y-1.5">
          <p className="text-xs font-medium text-text-secondary">
            {isCard ? t("payFrom") : t("payingWith")} <span className="text-text-muted font-normal">({t("optional")})</span>
          </p>
          {checkingAccounts.map((acc) => <AccountOption key={acc.id} acc={acc} />)}
